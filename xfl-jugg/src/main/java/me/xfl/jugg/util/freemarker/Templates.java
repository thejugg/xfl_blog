package me.xfl.jugg.util.freemarker;
...
public final class Templates {
	...
	static {
		// MAIN_CFG和MOBILE_CFG的模板目录在xfl-core/me.xfl.dragon.util.Shins类
		// 的方法setDirectoryForTemplateLoading中进行了初始化设置
	...
	private static boolean hasExpression(final Template template, final String expression, final TemplateElement templateElement) {
		...
		if (canonicalForm.startsWith(expression)) {
			LOGGER.log(Level.FINEST, "Template has expression[nodeName={0}, expression={1}]", new Object[] { templateElement.getNodeName(),
					expression });
	...
	public static Template getTemplate(final String templateDirName, final String templateName) {
		...
		try {
			try {
				...
				LOGGER.log(Level.SEVERE, "Can not load mobile template[templateDirName={0}, templateName={1}]", new Object[] {
