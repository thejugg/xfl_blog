...
import me.xfl.jugg.cache.CacheFactory;
import me.xfl.jugg.model.Pagination;
import me.xfl.jugg.repository.CompositeFilter;
import me.xfl.jugg.repository.Filter;
import me.xfl.jugg.repository.FilterOperator;
import me.xfl.jugg.repository.Projection;
...
import me.xfl.jugg.repository.jdbc.util.JdbcRepositories;
import me.xfl.jugg.repository.jdbc.util.JdbcUtil;
import me.xfl.jugg.util.Ids;
...
public final class JdbcRepository implements Repository {
	 ...
	 *             JSONException
	 */
	private void update(final String id, final JSONObject oldJsonObject, final JSONObject jsonObject, final List<Object> paramList,
	 ...
	 *             JSONException
	 */
	private void setUpdateProperties(final String id, final JSONObject needUpdateJsonObject, final List<Object> paramList,
			final StringBuilder sql) throws JSONException {
		...
		sql.append("update ").append(getName()).append(wildcardString).append(" where ").append(JdbcRepositories.OID).append("=")
				.append("?");
		paramList.add(id);
	}
	...
	public JSONObject get(final Query query) throws RepositoryException {
		...
		if (cacheEnabled) {
			ret = (JSONObject) CACHE.get(cacheKey);
			if (null != ret) {
				LOGGER.log(Level.FINER, "Got query result[cacheKey={0}] from repository cache[name={1}]", new Object[] { cacheKey,
						getName() });
				return ret;
			}
	 ...
	 *             json exception
	 */
	private void cacheQueryResults(final JSONArray results, final Query query) throws JSONException {
		String cacheKey;
		for (int i = 0; i < results.length(); i++) {
			...
			LOGGER.log(Level.FINER, "Added an object[cacheKey={0}] in repository cache[{1}] for default index[oId]", new Object[] {
					cacheKey, getName() });
			...
			for (final String[] index : indexes) {
				final Query futureQuery = new Query().setPageCount(1);
	 ...
	 *            sql
	 */
	private void getQuerySql(final int currentPageNum, final int pageSize, final StringBuilder selectSql, final StringBuilder filterSql,
	 ...
	 *             repository exception
	 */
	private void processCompositeFilter(final StringBuilder filterSql, final List<Object> paramList, final CompositeFilter compositeFilter)
			throws RepositoryException {
