...
public abstract class AbstractEventQueue {
	 ...
	 *             event exception
	 * @see AbstractEventListener#performAction(org.b3log.latke.event.AbstractEventQueue,
	 ...
	 * {@code hasChanged} method, then notify all of its listeners and then call
	 * the {@code clearChanged} method to indicate that this object has no
	 * longer changed.
	 * 
	 * @param event
	 *            the specified event
	...
	public void notifyListeners(final Event<?> event) throws EventException {
		...
		synchronized (this) {
			 ...
			 * needs synchronization, but notifying listeners does not (should
			 * not). The worst result of any potential race-condition here is
			 * that: 1) a newly-added listener will miss a notification in
			 * progress 2) a recently unregistered listener will be wrongly
	...
	/**
	 * Clears the listener list so that this object no longer has any
	 * listeners..
	 */
	 ...
	 * notified all of its listeners of its most recent change, so that the
	 * {@code hasChanged} method will now return {@code false}. This method is
	 * called automatically by the {@code notifyListeners} methods.
	 * 
	 * @see #notifyListeners()
	 ...
	 *         been called more recently than the {@code clearChanged} method on
